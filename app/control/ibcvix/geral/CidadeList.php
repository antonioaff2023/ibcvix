...
class CidadeList extends TPage
{
  ...
  public function __construct()
  {
    ...
    $this->setOrderCommand('estado->nome', '(SELECT nome from estados where id=cidades.id_estado)');
    $this->setLimit(4);
    // $this->setCriteria($criteria) // define a standard filter

    $this->addFilterField('id', '=', 'id'); // filterField, operator, formField
    $this->addFilterField('nome', 'like', 'nome'); // filterField, operator, formField
    $this->addFilterField('id_estado', '=', 'id_estado'); // filterField, operator, formField

    // creates the form
    $this->form = new BootstrapFormBuilder('form_search_Cidade');
    $this->form->setFormTitle("<span class = 'titulo-form'><strong>Cidade</strong></span>");
    ...
    $id_estado = new TDBUniqueSearch('id_estado', 'ibcvix', 'Estados', 'id', 'uf');
    $id_estado->setMinLength(0);
    $id_estado->setMask('{nome} ({uf})');

    // adicionar os campos
    //$this->form->addFields( [ new TLabel('Id') ], [ $id ] );
    $this->form->addFields([new TLabel('Nome')], [$nome]);
    $this->form->addFields([new TLabel('Estado')], [$id_estado]);
    ...
    $btn2 = $this->form->addActionLink(_t('New'), new TAction(['CidadeForm', 'onEdit'], ['register_state' => 'false']), 'fa:plus black');
    ...
    $column_id_estado = new TDataGridColumn('{estado->nome} ({estado->uf})', 'Estado', 'left');

    $column_id_estado->enableAutoHide(500);

    // add the columns to the DataGrid
    //$this->datagrid->addColumn($column_id);
    $this->datagrid->addColumn($column_nome);
    $this->datagrid->addColumn($column_id_estado);


    // creates the datagrid column actions
    //$column_id->setAction(new TAction([$this, 'onReload']), ['order' => 'id']);
    $column_nome->setAction(new TAction([$this, 'onReload']), ['order' => 'nome']);
    $column_id_estado->setAction(new TAction([$this, 'onReload']), ['order' => 'estado->nome']);
    ...
    $dropdown->addAction(_t('Save as PDF'), new TAction([$this, 'exportaGeral'], ['nome' => $cidade, 'estado' => $estado]), 'far:file-pdf red');
