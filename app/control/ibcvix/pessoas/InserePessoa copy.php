...
class InserePessoa extends TPage
{
    ...
    public function __construct($param)
    {
        ...
        $row = $this->form->addFields([new TLabel('Nome'), $nome], [new TLabel('Sobrenome'), $sobrenome]);
        $row->layout = ['col-sm-4', 'col-sm-4'];



        $row = $this->form->addFields([new TLabel('Nome completo'), $nome_completo], [new TLabel('Apelido'), $apelido]);
        $row->layout = ['col-sm-5', 'col-sm-3', 'col-sm-1'];

        $row = $this->form->addFields([new TLabel('Sexo'), $genero], [new TLabel('Data de nascimento'), $data_nascimento], [new TLabel('Nacionalidade'), $nacionalidade], [new TLabel('Estado Civil'), $estadocivil]);
        $row->layout = ['col-sm-1', 'col-sm-2', 'col-sm-3', 'col-sm-2'];

        $this->form->addFields([new TLabel('Pai'), $pai], [new TLabel('Mãe'), $mae])
            ->layout = ['col-sm-4', 'col-sm-4'];


        $row = $this->form->addFields([new TLabel('Categoria'), $categoria], [new TLabel('Cônjuge evangélico?'), $conjuge_cristao], [new TLabel('Cônjuge'), $nome_conjuge]);
