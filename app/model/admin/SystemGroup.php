...
class SystemGroup extends TRecord
{
    ...
    /**
     * Determina os grupos que podem acessar determinadas áreas do sistema dentro de um código
     */
    public function GruposAcesso(array $gruposPermitidos)
    {
        TTransaction::open('ibcvix');
        $grp_usr = SystemUserGroup::where('system_user_id', '=', TSession::getValue('userid'))->first();
        $grupo = (int) $grp_usr->system_group_id;
        TTransaction::close();
    
        return in_array($grupo, $gruposPermitidos);
    }
}
